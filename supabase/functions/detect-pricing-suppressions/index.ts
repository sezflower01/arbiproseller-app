...
// Non-pricing buckets we've observed from the live probe. If every category on an
// ERROR+LISTING_SUPPRESSED issue is in this set, it's a known non-pricing suppression
// (brand gate, missing attribute) and we do NOT flag it for admin review.
...
function classifyIssues(issues: any[]): Classified {
  ...
  try {
    ...
    await Promise.all(MARKETPLACES.map(async (mp) => {
      ...
      for (const a of list) {
        ...
        try {
          const { http_status: hs, body } = await callGetListingsItem(accessToken, sellerId, a.sku, mp.id);
          http_status = hs;
