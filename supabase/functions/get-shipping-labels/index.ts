...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Getting shipping labels for plan:", inboundPlanId, "shipment:", shipmentId);

    const accessToken = await getAccessToken(globalRefreshToken);

    // Get shipping labels with SigV4 signing
    const labelsPath = `/inbound/fba/2024-03-20/inboundPlans/${inboundPlanId}/shipments/${shipmentId}/deliveryWindowOptions`;
    ...
    console.log("Shipping labels response:", labelsResponse.status, labelsText);

    // Also try to get box labels
    const boxLabelsPath = `/inbound/fba/2024-03-20/inboundPlans/${inboundPlanId}/shipments/${shipmentId}/labels`;
    const boxLabelsQueryParams = "pageType=PackageLabel_Letter_6&labelType=BARCODE_2D&numberOfPackages=1";
    
    const boxLabelsResponse = await spApiSignedFetch({
      method: "GET",
      path: boxLabelsPath,
      queryParams: boxLabelsQueryParams,
      accessToken,
    });

    const boxLabelsText = await boxLabelsResponse.text();
    console.log("Box labels response:", boxLabelsResponse.status, boxLabelsText);

    let shippingLabelUrl: string | null = null;
    let boxLabelUrl: string | null = null;

    if (labelsResponse.ok) {
      try {
        const labelsData = JSON.parse(labelsText);
        shippingLabelUrl = labelsData.downloadURL || labelsData.downloadUrl || null;
      } catch {
        // ignore
      }
    }

    if (boxLabelsResponse.ok) {
      try {
        const boxLabelsData = JSON.parse(boxLabelsText);
        boxLabelUrl = boxLabelsData.downloadURL || boxLabelsData.downloadUrl || null;
      } catch {
        // ignore
      }
    }
    ...
    return new Response(JSON.stringify({
      ...
      shippingLabelUrl,
      boxLabelUrl,
      deliveryWindowOptions: labelsResponse.ok ? JSON.parse(labelsText) : null,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
