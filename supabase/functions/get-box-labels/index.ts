...
serve(async (req) => {
  ...
  try {
    ...
    const { inboundPlanId, shipmentId, pageType, labelType, numberOfPackages } = await req.json();
    console.log("Getting box labels for plan:", inboundPlanId, "shipment:", shipmentId);

    const accessToken = await getAccessToken(globalRefreshToken);

    // Try to get box labels with SigV4 signing
    ...
    console.log("Box labels response:", labelsResponse.status, labelsText);
    ...
    if (!labelsResponse.ok) {
      ...
      const altResponse = await spApiSignedFetch({
        method: "GET",
        path: altPath,
        queryParams: altQueryParams,
        accessToken,
      });

      const altText = await altResponse.text();
      console.log("Alternative box labels response:", altResponse.status, altText);

      if (!altResponse.ok) {
        return new Response(JSON.stringify({ 
          error: "Failed to get box labels", 
          details: labelsText 
        }), {
      ...
      return new Response(JSON.stringify({
        success: true,
        downloadUrl: altData.payload?.DownloadURL || altData.downloadURL,
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({
      success: true,
      downloadUrl: labelsData.downloadURL || labelsData.downloadUrl,
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });

  } catch (error) {
    console.error("Error getting box labels:", error);
    return new Response(JSON.stringify({ 
      error: "Failed to get box labels", 
