...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // detect-pricing-suppressions. Single-strike clear: one clean read here
    // is enough -- if the issue genuinely persists, the next detection pass
    // re-flags it anyway, so there's no need to wait for a second confirming
    // read before clearing (see pricing-suppression-core.ts for the same
    // decision on the nightly/queue-driven path).
    if (response.ok && !isPricingSuppressed) {
      try {
        const { data: assignment } = await supabase
          .from('repricer_assignments')
          .select('id, is_pricing_suppression, pricing_suppression_detected_at, pricing_suppression_raw_code, pricing_suppression_raw_message, pricing_suppression_categories, pricing_suppression_enforcement_actions, pricing_suppression_severity')
          .eq('user_id', user.id)
          .eq('sku', sku)
        ...
        // the next detection pass will pick up the clear either way.
      }
    }
