-- Repoint the 4-hour inventory-report cron from the per-user endpoint
-- to the new fan-out wrapper that loops over every connected user.
DO $$
DECLARE
  jid bigint;
BEGIN
  SELECT jobid INTO jid FROM cron.job WHERE jobname = 'sync-inventory-report-4h';
...
-- Trigger an immediate fan-out so the inventory valuation refreshes now.
SELECT net.http_post(
  url := 'https://mstibdszibcheodvnprm.supabase.co/functions/v1/sync-inventory-report-all',
  headers := jsonb_build_object(
    'Content-Type', 'application/json',
  ...
  body := jsonb_build_object('triggered_by', 'manual-immediate', 'time', now()),
  timeout_milliseconds := 300000
);