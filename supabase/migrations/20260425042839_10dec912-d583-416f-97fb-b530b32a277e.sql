...
-- 1) Add unresolved_date flag
ALTER TABLE public.fba_shipments
  ADD COLUMN IF NOT EXISTS unresolved_date boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_fba_shipments_unresolved_date
  ON public.fba_shipments (user_id, unresolved_date)
  WHERE unresolved_date = true;

-- 2) Clean up 144 contaminated rows: ship_date = '2026-04-24' came from Amazon's
--    lastUpdatedAt (the sync timestamp), not from a real ship date.
UPDATE public.fba_shipments
SET
  ship_date = NULL,
  received_date = CASE
    WHEN received_date = '2026-04-24'::date THEN NULL
    ELSE received_date
  END,
  unresolved_date = true
WHERE ship_date = '2026-04-24'::date;

-- Also catch any other rows where no real date source exists
UPDATE public.fba_shipments
SET unresolved_date = true
WHERE ship_date IS NULL
  AND received_date IS NULL
  AND (shipment_name IS NULL OR NOT (shipment_name ~ '\d{1,2}/\d{1,2}/\d{2,4}'))
  AND confirmed_need_by_date IS NULL
  AND unresolved_date = false;

-- 3) Refactor the report RPC: drop last_updated_date from the date chain,
--    return unresolved_date, and accept p_include_unresolved.
DROP FUNCTION IF EXISTS public.get_shipment_accounting_period(date, date);
DROP FUNCTION IF EXISTS public.get_shipment_accounting_period(date, date, boolean);

CREATE OR REPLACE FUNCTION public.get_shipment_accounting_period(
  p_start date,
  p_end date,
  p_include_unresolved boolean DEFAULT false
)
RETURNS TABLE(
  shipment_id text,
  shipment_name text,
...
AS $function$
  ...
  shipments_with_date AS (
    SELECT
      ...
      sh.shipment_status,
      sh.unresolved_date,
      -- NOTE: sh.last_updated_date is intentionally EXCLUDED — Amazon returns
      -- the API call timestamp there, not a real ship date. Using it caused
      -- 144 shipments to be mis-bucketed into April 2026.
      COALESCE(
        sh.ship_date,
        sh.received_date,
  ...
  s_filtered AS (
    ...
    WHERE shipment_date >= p_start
      ...
      AND (p_include_unresolved OR unresolved_date = false)
  ),
  item_rows AS (
    SELECT
      i.shipment_id,
  ...
  items AS (
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY source_priority, source_at DESC
      LIMIT 1
    ) uc ON true
  ...
  ship_asin_units AS (
    SELECT
      ...
      SUM(ir.quantity_shipped) AS units
    FROM item_rows ir
...
-- 4) Update list_shipments_needing_date_sync to include unresolved rows so the
--    user can re-run Sync Dates if SP-API improves later.
DROP FUNCTION IF EXISTS public.list_shipments_needing_date_sync(uuid, integer);

CREATE OR REPLACE FUNCTION public.list_shipments_needing_date_sync(
  p_user_id uuid,
...
AS $function$
  ...
  WHERE sh.user_id = p_user_id
    AND (sh.dates_synced_at IS NULL OR sh.unresolved_date = true)
  ORDER BY
...
-- 5) Helper: count unresolved-date shipments for the current user
CREATE OR REPLACE FUNCTION public.count_unresolved_shipment_dates()
RETURNS integer
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  SELECT COUNT(*)::int
  FROM public.fba_shipments
  WHERE user_id = auth.uid()
    AND unresolved_date = true;
$function$;

-- 6) Manual ship-date assignment for unresolved shipments (owner or admin)
CREATE OR REPLACE FUNCTION public.set_shipment_manual_ship_date(
  p_shipment_id text,
  p_ship_date date
)
RETURNS void
...
BEGIN
  ...
  SET
    ...
    unresolved_date = (p_ship_date IS NULL),
    dates_synced_at = COALESCE(dates_synced_at, now())
  WHERE shipment_id = p_shipment_id;
END;
$function$;

-- 7) RPC to list unresolved shipments for the current user (review panel)
CREATE OR REPLACE FUNCTION public.list_unresolved_shipments(p_limit integer DEFAULT 200)
RETURNS TABLE(
  shipment_id text,
  shipment_name text,
  shipment_status text,
...
AS $function$
  SELECT
    ...
    sh.created_at,
    COALESCE((
      SELECT SUM(quantity_shipped)::bigint
      FROM public.fba_shipment_items
      WHERE user_id = sh.user_id AND shipment_id = sh.shipment_id
    ), 0) AS units_shipped
  FROM public.fba_shipments sh
  WHERE sh.user_id = auth.uid()
    AND sh.unresolved_date = true
