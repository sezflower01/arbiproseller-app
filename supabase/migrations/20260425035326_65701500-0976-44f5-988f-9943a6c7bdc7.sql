-- 1. Add real date fields to fba_shipments. These come from Amazon SP-API getShipment / getInboundShipmentItems
ALTER TABLE public.fba_shipments
  ...
  ADD COLUMN IF NOT EXISTS dates_synced_at timestamp with time zone;

CREATE INDEX IF NOT EXISTS idx_fba_shipments_ship_date ON public.fba_shipments(user_id, ship_date) WHERE ship_date IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_fba_shipments_dates_synced_at ON public.fba_shipments(user_id, dates_synced_at);

-- 2. Update the RPC to use ship_date / received_date as the primary date sources
...
-- 3. Helper RPC: list shipments needing date sync (called by edge function in batches)
CREATE OR REPLACE FUNCTION public.list_shipments_needing_date_sync(p_user_id uuid, p_limit int DEFAULT 25)
...
AS $$
  ...
  WHERE sh.user_id = p_user_id
    AND sh.dates_synced_at IS NULL
