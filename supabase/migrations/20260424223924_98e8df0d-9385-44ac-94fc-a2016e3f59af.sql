...
AS $function$
  ...
  asin_revenue AS (
    SELECT
      aw.shipment_id,
      aw.asin,
      COALESCE(SUM(ABS(COALESCE(f.sales, 0))), 0) AS revenue
    FROM asin_window aw
    LEFT JOIN public.financial_events_cache f
      ON f.user_id = auth.uid()
     AND f.event_type = 'shipment'
     AND f.asin = aw.asin
     AND f.event_date >= aw.window_start
     AND f.event_date <  aw.window_end
