...
DECLARE _id BIGINT;
BEGIN
  SELECT jobid INTO _id FROM cron.job WHERE jobname = 'health-retry-worker-5m';
  IF _id IS NOT NULL THEN PERFORM cron.unschedule(_id); END IF;
END $$;

SELECT cron.schedule(
  'health-retry-worker-5m',
  '*/5 * * * *',
  $$
  SELECT net.http_post(
    url := 'https://mstibdszibcheodvnprm.supabase.co/functions/v1/health-retry-worker',
