...
AS $function$
  ...
  avg_price AS (
    SELECT COALESCE(AVG(
      CASE WHEN COALESCE(total_sale_amount, 0) > 0 THEN total_sale_amount / GREATEST(quantity, 1)
           WHEN effective_price > 0 THEN effective_price
           ELSE NULL
      END
    ), 0) AS avg_unit_price
    FROM base
    WHERE effective_price > 0 OR COALESCE(total_sale_amount, 0) > 0
  ),
  latest_unit_cost AS (
  ...
  SELECT
    COALESCE(SUM(
      CASE WHEN COALESCE(total_sale_amount, 0) > 0 THEN total_sale_amount
           ...
           ELSE (SELECT avg_unit_price FROM avg_price) * quantity
      END
    ), 0) AS sales,
