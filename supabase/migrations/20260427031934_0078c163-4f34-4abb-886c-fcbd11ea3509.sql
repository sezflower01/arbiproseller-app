...
CREATE OR REPLACE FUNCTION public.get_reimbursement_pl_debug(p_year integer)
RETURNS TABLE(
  month_num integer,
  generic_reimbursements numeric,
  reversal_reimbursement numeric,
  free_replacement_refund_items numeric,
  final_displayed_reimbursements numeric
)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  ...
  SELECT
    months.m AS month_num,
    COALESCE(SUM(COALESCE(b.reimbursements, 0)), 0) AS generic_reimbursements,
    COALESCE(SUM(ABS(COALESCE(b.reversal_reimbursement, 0))), 0) AS reversal_reimbursement,
    COALESCE(SUM(ABS(COALESCE(b.free_replacement_refund_items, 0))), 0) AS free_replacement_refund_items,
    COALESCE(SUM(
      COALESCE(b.reimbursements, 0)
      + ABS(COALESCE(b.reversal_reimbursement, 0))
      + ABS(COALESCE(b.free_replacement_refund_items, 0))
    ), 0) AS final_displayed_reimbursements
  FROM months
  LEFT JOIN base b ON EXTRACT(MONTH FROM b.event_date)::int = months.m
  GROUP BY months.m
  ORDER BY months.m;
