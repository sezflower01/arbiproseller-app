...
DROP FUNCTION IF EXISTS public.get_smart_fallback_daily_totals(text, text);

CREATE OR REPLACE FUNCTION public.get_smart_fallback_daily_totals(start_ts text, end_ts text)
 RETURNS TABLE(day date, so_orders bigint, so_units numeric, so_sales numeric, so_refunds numeric, so_fees numeric, so_cogs numeric, so_promo_rebates numeric, fec_orders bigint, fec_units bigint, fec_sales numeric, fec_refunds numeric, fec_fees numeric, fec_promo_rebates numeric, fec_shipping_credits numeric, fec_gift_wrap_credits numeric)
 LANGUAGE sql
...
AS $function$
  ...
  so_per_day AS (
    SELECT
      ...
      COALESCE(SUM(ABS(COALESCE(s.promotion_discount, 0))), 0) AS so_promo_rebates
  ...
  order_purchase_date AS (
    SELECT
      s.order_id,
      MIN(s.order_date::date) AS purchase_day
    FROM public.sales_orders s
    CROSS JOIN bounds b
    WHERE s.user_id = auth.uid()
      AND s.order_id IS NOT NULL AND s.order_id <> ''
      AND s.order_date >= (b.start_d - INTERVAL '120 days')
      AND s.order_date <  (b.end_d   + INTERVAL '60 days')
    GROUP BY s.order_id
  ),
  fec_per_day AS (
    SELECT
      COALESCE(opd.purchase_day, f.event_date::date) AS day,
    ...
    CROSS JOIN bounds b
    LEFT JOIN order_purchase_date opd ON opd.order_id = f.amazon_order_id
    WHERE f.user_id = auth.uid()
      AND f.event_date >= (b.start_d - INTERVAL '60 days')
      AND f.event_date <  (b.end_d   + INTERVAL '60 days')
      AND COALESCE(opd.purchase_day, f.event_date::date) >= b.start_d
      AND COALESCE(opd.purchase_day, f.event_date::date) <  b.end_d
    GROUP BY 1
  )
  SELECT
    d.day,
    ...
    COALESCE(s.so_promo_rebates, 0) AS so_promo_rebates,
