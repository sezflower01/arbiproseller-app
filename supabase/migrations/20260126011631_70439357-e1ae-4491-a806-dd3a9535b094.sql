-- Fix get_sellerboard_period_totals to match Excel logic:
-- Exclude pending/unshipped orders UNLESS they have sold_price > 0 (settled data)
...
AS $function$
  ...
  -- MATCH EXCEL EXPORT: Exclude Pending/Unshipped orders UNLESS they have sold_price > 0
  base AS (
    SELECT *
    ...
    WHERE s.user_id = auth.uid()
      ...
      -- CRITICAL: Exclude pending/unshipped orders UNLESS they have settled data (sold_price > 0)
      -- This matches Excel export: pending orders with Financial Events data are included
      AND NOT (
