...
AS $function$
  ...
  s AS (
    -- Anchor each shipment to its CREATION date (the meaningful business date),
    -- falling back to confirmed_need_by_date, then to the first inbound-fee
    -- posted date as a last resort. This is the SAME date used for filtering,
    -- monthly grouping, and display — guaranteeing consistency.
    SELECT
      sh.shipment_id,
      sh.shipment_name,
      sh.shipment_status,
  ...
  all_ship_asin AS (
    ...
    FROM public.fba_shipment_items i
    JOIN public.fba_shipments sh ON sh.shipment_id = i.shipment_id
    LEFT JOIN fee_min_date fmd ON fmd.shipment_id = sh.shipment_id
    WHERE i.user_id = auth.uid()
  ...
  -- financial_events_cache.asin actually stores SellerSKU.
  -- Resolve the real ASIN by joining FEC -> sales_orders on amazon_order_id.
  fec_with_real_asin AS (
    SELECT
  ...
  asin_revenue AS (
    SELECT
      aw.shipment_id,
      aw.asin,
      COALESCE(SUM(ABS(COALESCE(fr.sales, 0))), 0) AS revenue
    FROM asin_window aw
    LEFT JOIN fec_with_real_asin fr
      ON fr.real_asin = aw.asin
     AND fr.event_date >= aw.window_start
     AND fr.event_date <  aw.window_end
    GROUP BY aw.shipment_id, aw.asin
  ),
  shipment_revenue AS (
    SELECT
      shipment_id,
