CREATE OR REPLACE FUNCTION public.get_smart_fallback_daily_totals(start_ts text, end_ts text)
 RETURNS TABLE(day date, so_orders bigint, so_units numeric, so_sales numeric, so_refunds numeric, so_fees numeric, so_cogs numeric, fec_orders bigint, fec_units bigint, fec_sales numeric, fec_refunds numeric, fec_fees numeric, fec_promo_rebates numeric, fec_shipping_credits numeric, fec_gift_wrap_credits numeric)
 LANGUAGE sql
...
AS $function$
  ...
  -- Map each FEC event to the ORDER'S PURCHASE DATE (sales_orders.order_date).
  -- Fall back to the event_date only when the order is not present in sales_orders.
  -- This aligns Smart Fallback with Sellerboard, which groups by purchase date,
  -- and prevents prior-period orders that settle in the current period from
  -- inflating MTD/period totals.
  fec_attributed AS (
    SELECT
      f.*,
      COALESCE(
        (SELECT MIN(s2.order_date::date)
         FROM public.sales_orders s2
         WHERE s2.user_id = f.user_id
           AND s2.order_id = f.amazon_order_id),
        f.event_date::date
      ) AS purchase_day
    FROM public.financial_events_cache f
    CROSS JOIN bounds b
    WHERE f.user_id = auth.uid()
      -- Pull a wider FEC window so events whose purchase date falls in [start,end)
      -- but settle later are still attributed correctly. 60-day lookback covers
      -- typical Amazon settlement lag.
      AND f.event_date >= (b.start_d - INTERVAL '60 days')
      AND f.event_date <  (b.end_d   + INTERVAL '60 days')
  ),
  fec_per_day AS (
    SELECT
      f.purchase_day AS day,
    ...
    FROM fec_attributed f
    CROSS JOIN bounds b
    WHERE f.purchase_day >= b.start_d
      AND f.purchase_day <  b.end_d
    GROUP BY 1
  )
  SELECT
    d.day,
