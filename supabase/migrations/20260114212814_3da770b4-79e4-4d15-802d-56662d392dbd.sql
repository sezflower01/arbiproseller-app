-- Update the get_settled_period_totals function to return additional fields for Net Sales calculation
-- Net Sales = Gross Sales - Promotional Rebates - Shipping Credits - Gift Wrap Credits
...
CREATE OR REPLACE FUNCTION public.get_settled_period_totals(start_ts timestamp with time zone, end_ts timestamp with time zone)
 RETURNS TABLE(
   ...
   cogs numeric,
   -- NEW: fields for Net Sales calculation (Sellerboard-style)
...
AS $function$
  ...
  SELECT
    ...
    -- NEW: Sum promotional rebates (stored as negative in Amazon data, we want absolute value)
