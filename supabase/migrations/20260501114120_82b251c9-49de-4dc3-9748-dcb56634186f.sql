-- Fix sync-inventory-report cron: previous job sent only apikey header (rejected as Invalid JWT).
-- Replace with proper Authorization Bearer header so the function accepts the call.
