-- 1. Add restocking_fee column
ALTER TABLE public.financial_events_cache
ADD COLUMN IF NOT EXISTS restocking_fee numeric DEFAULT 0;

COMMENT ON COLUMN public.financial_events_cache.restocking_fee IS
  'Restocking fees charged to customers on returns and credited to the seller. Sourced from SP-API Finances RefundEvent ItemChargeAdjustmentList where ChargeType = RestockingFee.';

-- 2. Drop and recreate get_monthly_pl_breakdown with new return type
DROP FUNCTION IF EXISTS public.get_monthly_pl_breakdown(integer);

CREATE FUNCTION public.get_monthly_pl_breakdown(p_year integer)
 RETURNS TABLE(
   month_num integer,
   sales numeric, refunds numeric, reimbursements numeric,
   shipping_credits numeric, shipping_credit_refunds numeric,
   gift_wrap_credits numeric, gift_wrap_credit_refunds numeric,
   promotional_rebates numeric, promotional_rebate_refunds numeric,
   other_income numeric, liquidations numeric, intl_markets numeric,
   referral_fees numeric, fba_fees numeric,
   variable_closing_fees numeric, fixed_closing_fees numeric,
   fba_inbound_fees numeric, fba_storage_fees numeric,
   fba_removal_fees numeric, fba_disposal_fees numeric,
   fba_long_term_storage_fees numeric, fba_customer_return_fees numeric,
   digital_services_fee numeric, fba_inbound_convenience_fee numeric,
   other_fees numeric, liquidations_brokerage_fee numeric,
   re_commerce_grading_charge numeric, compensated_clawback numeric,
   hrr_non_apparel numeric, warehouse_lost numeric, warehouse_damage numeric,
   reversal_reimbursement numeric, free_replacement_refund_items numeric,
   sales_tax_collected numeric, marketplace_facilitator_tax numeric,
   sales_tax_refunds numeric, marketplace_facilitator_tax_refunds numeric,
   shipping_chargeback numeric, shipping_chargeback_refund numeric,
