-- fn_detect_bb_drop() (the trigger behind bb_price_alerts) fired on ANY
-- buy-box price drop snapshot, with no check for whether the seller
-- actually has a sellable listing in that marketplace. Confirmed live:
-- ASIN B07YD5D8HX has a BR assignment with intl_listing_status =
-- "DISCOVERABLE" and marketplace_sellable = false (shows up in Amazon's
-- BR catalog, but this seller has no real/buyable listing there -- same
-- for CA), yet BR competitor snapshots keep getting fetched daily and
-- generating "Buy Box Price Alerts" the user can't act on since there's
-- nothing to reprice.
--
-- Fix: skip alert creation unless the marketplace is US (always eligible)
-- or the assignment's marketplace_sellable = true -- the same "is this a
-- real market" definition already used for billing (get_managed_listings_
-- counts) and the repricer assignments table itself.
