-- Raises the bar for what creates a Buy Box Price Alert. Previously any
-- drop >= $0.02 OR >= 0.5% qualified, which meant most alerts were tiny,
-- non-actionable moves the repricer already handles on its own -- users
-- ended up seeing the same low-value "small price move" message
-- repeatedly.
--
-- New rule: only create an alert when the drop is >= 1%, OR the seller is
-- now priced above the new Buy Box price (genuinely losing the sale
-- regardless of how small the specific move was). This requires moving
-- the my_price lookup earlier so it can gate the decision, not just
-- decorate the alert row afterward.
...
BEGIN
  ...
  SELECT (a.marketplace = 'US' OR a.marketplace_sellable = true) INTO v_is_sellable
  ...
  IF v_is_sellable IS NOT TRUE THEN
    RETURN NEW;
  END IF;
  ...
  -- No real drop at all -- skip before even looking up my_price.
  IF v_drop_abs <= 0 THEN
    RETURN NEW;
  END IF;
  ...
  v_losing_bb := (v_my_price IS NOT NULL AND v_my_price > NEW.buybox_price);

  IF v_drop_pct < v_threshold_pct AND NOT v_losing_bb THEN
    RETURN NEW;
  END IF;
