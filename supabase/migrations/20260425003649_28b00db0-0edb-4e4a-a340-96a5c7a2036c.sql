...
AS $function$
  ...
  -- Per-shipment ASIN units (preserve units for proportional allocation)
  ship_asin_units AS (
    SELECT
      i.shipment_id,
      i.asin,
  ...
  -- Total revenue available within each (asin, window)
  ...
  -- Sum of units across all split shipments that share the same (asin, window)
  -- This is the denominator for proportional allocation.
  ...
  -- Allocate revenue proportionally by shipped units within each (asin, window) group
  asin_revenue AS (
    SELECT
      awr.shipment_id,
      awr.asin,
