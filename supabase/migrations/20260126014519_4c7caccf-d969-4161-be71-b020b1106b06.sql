-- Fix RPC settlement logic to match Excel export and UI blocks
-- Include pending orders if they have EITHER sold_price > 0 OR total_sale_amount > 0
...
AS $function$
  ...
  -- MATCH EXCEL EXPORT: Exclude Pending/Unshipped orders UNLESS they have settled data
  base AS (
    SELECT *
    ...
    WHERE s.user_id = auth.uid()
      ...
      -- FIXED: Exclude pending/unshipped orders UNLESS they have settled data from Financial Events
      -- Include if sold_price > 0 OR total_sale_amount > 0 (BOTH are settled from Financial Events)
      AND NOT (
        COALESCE(s.order_status, '') IN ('Pending', 'Unshipped', 'PendingAvailability', 'PartiallyShipped')
