...
-- Create a new RPC function for Sellerboard-style totals using PurchaseDate from sales_orders
...
AS $function$
  ...
  -- Base query: sales_orders by purchase date (order_date), settled only, excluding cancelled
  base AS (
    SELECT *
  ...
  -- Aggregate by ASIN for COGS calculation
  asin_totals AS (
    SELECT
      NULLIF(asin, '') AS asin,
  ...
  -- Get latest unit cost from created_listings
  latest_unit_cost AS (
  ...
  SELECT
    ...
    COALESCE(
      (
        SELECT SUM(at.units * COALESCE(luc.unit_cost, 0))
        FROM asin_totals at
        LEFT JOIN latest_unit_cost luc ON luc.asin = at.asin
      ),
      0
    ) AS cogs
  FROM base;
