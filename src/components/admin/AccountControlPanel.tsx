...
export default function AccountControlPanel() {
  ...
  const [actionDialog, setActionDialog] = useState<{ action: string; user: UserAccount } | null>(null);
  ...
  const executeAction = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("admin-manage-account", {
      body: { action: currentAction, user_id: currentUserId, reason: reason || undefined },
    });
  ...
  const filtered = users.filter((u) => {
    ...
    return u.email?.toLowerCase().includes(q) || u.first_name?.toLowerCase().includes(q) || u.last_name?.toLowerCase().includes(q) || u.account_status?.toLowerCase().includes(q);
  });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="relative">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by email, name, or status…" className="pl-10" />
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {filtered.map((u) => {
                ...
                return (
                  <div key={u.id} className="rounded-xl border border-border bg-card p-4 hover:bg-accent/20 transition-colors">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-wrap gap-3 mt-2 text-xs text-muted-foreground">
                          <span>Subscription: <span className="font-medium text-foreground">{u.subscription?.status || "none"}{u.subscription?.cancel_at_period_end && " (cancelling)"}</span></span>
                          <span>Plan: <span className="font-medium text-foreground">{u.subscription?.plan_id || "—"}</span></span>
                          <span>Repricer: <span className={`font-medium ${u.repricer?.scheduler_enabled ? "text-green-600" : "text-muted-foreground"}`}>{u.repricer?.scheduler_enabled ? "ON" : "OFF"}</span></span>
                          <span>Amazon: <span className={`font-medium ${activeAuths > 0 ? "text-green-600" : "text-muted-foreground"}`}>{activeAuths > 0 ? `${activeAuths} active` : "disconnected"}</span></span>
                        </div>
                        {u.account_status_reason && u.account_status !== "active" && (
                          <p className="text-xs text-muted-foreground mt-1 italic">Reason: {u.account_status_reason}</p>
                        )}
                      </div>
                      {!isSelf && (
                          ...
                          {(u.account_status === "active" || u.account_status === "paused") && (
                            <>
                              {u.account_status === "active" && (
                                <Button variant="ghost" size="sm" className="text-yellow-600 hover:bg-yellow-50" onClick={() => setActionDialog({ action: "pause", user: u })}>
                                  <Pause className="h-3.5 w-3.5 mr-1" />Pause
                                </Button>
                              )}
                              <Button variant="ghost" size="sm" className="text-red-600 hover:bg-red-50" onClick={() => setActionDialog({ action: "suspend", user: u })}>
                                <Ban className="h-3.5 w-3.5 mr-1" />Suspend
                              </Button>
                            </>
                          )}
                          {(u.account_status === "suspended" || u.account_status === "paused") && (
                            <Button variant="ghost" size="sm" className="text-green-600 hover:bg-green-50" onClick={() => setActionDialog({ action: "restore", user: u })}>
                              <RotateCcw className="h-3.5 w-3.5 mr-1" />Restore
                            </Button>
                          )}
                          {u.account_status !== "deleted" && (
                            <Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10" onClick={() => setActionDialog({ action: "delete", user: u })}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3">
              ...
              <p className="font-medium text-foreground">User: {actionDialog?.user.email}</p>
              <div className="pt-2">
                <label className="text-sm font-medium text-foreground mb-1 block">Reason (optional)</label>
                <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g. Chargeback, abuse, requested by user…" rows={2} />
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
