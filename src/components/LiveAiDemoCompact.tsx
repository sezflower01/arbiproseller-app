...
// Reliable product imagery — Unsplash photos matched to each product category.
// Amazon's images.amazon.com/images/P/{ASIN} CDN is unreliable for many ASINs and often returns blanks.

// Large pool of diverse products — the demo cycles through these over a 60-minute loop
// so users see fresh ASINs/images continuously without obvious repetition.
...
const _legacySeed: DemoItem[] = [
  {
    id: "1",
    asin: "B0G54JL52W",
    title:
      "POP TV: Stranger Things - Dustin Henderson (Season 5) Funko Vinyl Figure (Bundled with Compatible Box Protector Case), Multicolor, 3.75 inches",
    image:
      "https://images.unsplash.com/photo-1608889335941-32ac5f2041b9?w=200&h=200&fit=crop&auto=format",
    current: 28.87,
    buyBox: null,
    evaluatedAt: Date.now() - 10_000,
  ...
  },
  {
    id: "2",
    asin: "B0GNC6CRCD",
    title: "2026 Topps Series 1 MLB Baseball Fat Pack",
    image:
      "https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=200&h=200&fit=crop&auto=format",
    current: 12.99,
    buyBox: null,
    evaluatedAt: Date.now() - 13_000,
    actionAt: Date.now() - 13_000,
    actionType: "no_change",
  },
  {
    id: "3",
    asin: "B0GGMLFGPG",
    title: "Kinder's Rubs & Seasonings - 1 bottle (Bourbon Peach 5.8oz)",
    image:
      "https://images.unsplash.com/photo-1599909533042-2e88b29c8c4d?w=200&h=200&fit=crop&auto=format",
    current: 13.49,
    buyBox: 15.99,
  ...
  },
  {
    id: "4",
    asin: "B0GT62JDVT",
    title: "2026 Topps Heritage Baseball - Mega Box - Factory Sealed - Made in USA",
    image:
      "https://images.unsplash.com/photo-1567593810070-7a3d471af022?w=200&h=200&fit=crop&auto=format",
    current: 88.89,
    buyBox: null,
    evaluatedAt: Date.now() - 17_000,
    actionAt: Date.now() - 17_000,
    actionType: "no_change",
  },
  {
    id: "5",
    asin: "B0FS8378QJ",
    title:
      "POP TV: Arcane League of Legends - Jinx Funko Vinyl Figure (Bundled with Compatible Box Protector Case), Multicolor, 3.75 inches",
    image:
      "https://images.unsplash.com/photo-1635805737707-575885ab0820?w=200&h=200&fit=crop&auto=format",
    current: 34.95,
    buyBox: null,
    evaluatedAt: Date.now() - 19_000,
    actionAt: Date.now() - 19_000,
    actionType: "no_change",
  },
  {
    id: "6",
    asin: "B0DQVSH65Z",
    title:
      "Lang Companies, Wild At Heart Deluxe 2026 Planner, 8.25'' X 9.5'', Weekly Calendar with Stapled Binding",
    image:
      "https://images.unsplash.com/photo-1506784983877-45594efa4cbe?w=200&h=200&fit=crop&auto=format",
    current: 19.01,
    buyBox: 18.99,
...
// Synthetic but realistic explanation details — mirrors the depth of the historical AI Insights view.
// Updated to show Gemini AI involvement in decision explanations.
...
const LiveAiDemoCompact = () => {
  ...
  // Cycle pointer — walks through productPool over the 60-minute loop
  ...
  useEffect(() => {
    // ~6s tick → ~600 events over a 60-minute cycle
    const TICK_MS = 6000;
    ...
    const t = window.setInterval(() => {
      setItems((prev) => {
        ...
        // Reset cycle every 60 minutes so it loops indefinitely
        if (tsNow - cyc.startedAt > 60 * 60 * 1000) {
          cyc.poolIndex = 0;
          cyc.startedAt = tsNow;
        }

        // 70%: introduce a NEW product from the pool. 30%: re-evaluate a visible one.
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-xl overflow-hidden">
      ...
      <div className="px-3 pt-2.5">
        <p className="text-[9px] uppercase tracking-wider text-gray-500">Simulation</p>
      </div>
        ...
        {items.slice(0, 6).map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "text-left rounded-xl border border-white/10 bg-white/[0.03] p-2.5 transition-all duration-500 cursor-pointer",
                flashing
            ...
            >
              <div className="flex items-start gap-2 mb-1.5">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white overflow-hidden">
                  <img
                    src={item.image}
                    alt=""
                    loading="lazy"
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <p className="line-clamp-1 text-[10px] font-medium text-white leading-tight flex-1 min-w-0">
                      {item.title}
                    </p>
                    <ChevronDown
                      className={cn(
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                    <span className="font-mono text-[9px] text-gray-500">{item.asin}</span>
                    <span
                      className={cn(
              ...
              <p className="mt-1 text-[9px] text-gray-500 leading-tight line-clamp-1">
                {item.actionType === "price_changed"
              ...
              {isExpanded && (
                <div className="mt-2 pt-2 border-t border-white/10 space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-200">
                  <div className="flex items-start gap-1.5">
                    <span className="text-[8px] uppercase tracking-wider text-gray-500 w-14 shrink-0 mt-0.5">Reason</span>
                    <span className="text-[10px] text-white leading-tight">{details.reason}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] uppercase tracking-wider text-gray-500 w-14 shrink-0">Anchor</span>
                    <span className="text-[10px] text-muted-foreground">{details.anchor}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] uppercase tracking-wider text-gray-500 w-14 shrink-0">Strategy</span>
                    <span className="font-mono text-[9px] text-primary">{details.strategy}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] uppercase tracking-wider text-gray-500 w-14 shrink-0">ROI floor</span>
                    <span className="text-[10px] text-muted-foreground tabular-nums">
                      ${details.roiFloor.toFixed(2)}
                      <span className={cn(
                        "ml-1.5 text-[9px]",
                        details.vsFloorPct >= 0 ? "text-emerald-300" : "text-rose-300"
                      )}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-[8px] uppercase tracking-wider text-gray-500 w-14 shrink-0">Guards</span>
      ...
      {/* Stats footer */}
      <div className="border-t border-white/5 bg-white/[0.02] px-3 py-2.5">
        <div className="grid grid-cols-5 gap-1.5">
          <div className="text-center">
            <p className="font-display text-base font-bold text-white tabular-nums leading-none">
              {stats.total}
            </p>
            <p className="text-[8px] text-muted-foreground mt-0.5 leading-tight">Total Evaluations</p>
          </div>
          <div className="text-center">
            <p className="font-display text-base font-bold text-rose-300 tabular-nums leading-none">
              {stats.bbLost}
            </p>
          ...
          <div className="text-center">
            <p className="font-display text-base font-bold text-amber-300 tabular-nums leading-none">
              {stats.constrained}
            </p>
            <p className="text-[8px] text-muted-foreground mt-0.5 leading-tight">Safety Protections</p>
          </div>
          <div className="text-center">
            <p className="font-display text-base font-bold text-emerald-300 tabular-nums leading-none">
              {stats.winners}
            </p>
            <p className="text-[8px] text-muted-foreground mt-0.5 leading-tight">Winning Positions</p>
          </div>
        </div>
        <p className="mt-2 pt-2 border-t border-white/5 text-center text-[9px] text-gray-500">
