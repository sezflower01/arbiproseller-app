...
// Re-themed copy of ProductLibraryBanner.tsx. Copy/data (proofLabels,
// needBuyAgainPreviewItems, subCards) are byte-for-byte identical — only the
// background wash, glow colors, and text/border classes changed. The CTA
// button was standardized to the default primary Button style (was a bespoke
// violet->fuchsia gradient) to match the single button language used
// elsewhere in the InventoryHub theme.
...
const ProductLibraryBannerThemed = () => {
  ...
  return (
    <section className="relative overflow-hidden py-20 md:py-28">
      {/* Light indigo/violet premium wash */}
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-50 via-violet-50 to-blue-50" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_10%,hsl(250,75%,68%,0.14),transparent_55%)]" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_85%_90%,hsl(220,75%,68%,0.12),transparent_55%)]" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet-500/15 border border-violet-400/25 text-violet-700 text-sm font-medium backdrop-blur-sm">
          ...
          <div className="flex flex-wrap justify-center gap-2 pt-4">
            {proofLabels.map((label) => (
              <span
                key={label}
                className="px-3 py-1.5 rounded-full bg-card/80 border border-border text-xs font-medium text-foreground backdrop-blur-sm"
              >
                {label}
              </span>
            ))}
        ...
        <div className="grid md:grid-cols-2 gap-5 lg:gap-6 max-w-6xl mx-auto">
          {subCards.map((card) => {
            ...
            return (
              <div
                key={card.badge}
                className="group relative overflow-hidden rounded-2xl border border-border bg-card/70 backdrop-blur-sm p-7 md:p-8 hover:border-primary/30 hover:bg-card transition-all duration-300"
              >
                {/* Card glow */}
                <div
                ...
                <div className="relative z-10 space-y-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className={`p-3 rounded-xl ${card.iconBg} border border-border`}>
                  ...
                  {card.badge === "03" ? (
                    <div className="pt-2">
                      {!showNeedBuyAgainPreview ? (
                        <div className="rounded-2xl border border-border bg-muted/50 px-4 py-5 text-foreground/80">
                          <div className="flex items-center gap-3 text-sm font-medium">
                            <div className="h-4 w-4 rounded-full border-2 border-border border-t-orange-500 animate-spin" />
                            <span>Calculating replenishment...</span>
                          </div>
                        </div>
                      ) : (
                        <div className="rounded-2xl border border-border bg-muted/50 p-3 md:p-4">
                          <div className="space-y-3 max-h-[560px] overflow-y-auto pr-1">
                            {needBuyAgainPreviewItems.map((item) => (
                              <div
                                key={item.asin}
                                className="rounded-xl border border-border bg-card/60 p-3 text-left"
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <p className="mt-1 text-xs font-mono text-muted-foreground">{item.asin}</p>
                                    <p className="mt-2 text-xs text-muted-foreground">Stock: {item.stock}</p>
                                    <p className="text-xs text-muted-foreground">Sales: {item.sales}</p>
                                  ...
                                  <div className="w-20 shrink-0 text-right">
                                    ...
                                    <div className="mt-1 text-[11px] uppercase tracking-[0.18em] text-orange-600">
                                      to order
                                    </div>
                                  </div>
                                </div>
                  ...
                  {card.supportingLine && (
                    <p className="text-sm text-muted-foreground italic pt-2 border-t border-border mt-4">
                      {card.supportingLine}
                    </p>
                  )}
                </div>
...
export default ProductLibraryBannerThemed;
