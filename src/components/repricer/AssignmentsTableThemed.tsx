...
export default function AssignmentsTableThemed({ rules, onViewOffers, marketplace = "US", onMarketplaceChange, onViewAppliedSuggestions, isAdmin = false }: AssignmentsTableProps) {
           ...
           {isAdmin && (() => {
             const allChip = { value: "ALL", label: "All", color: "bg-muted text-muted-foreground border-transparent", activeColor: "bg-primary text-primary-foreground shadow-md", icon: null as React.ReactNode };
              const operationalIssues = [
                { value: "blocked_review_soon", label: "🟡 Review Soon", color: "bg-amber-100 text-amber-800 border-amber-300", activeColor: "bg-amber-500 text-white shadow-md shadow-amber-500/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
                { value: "blocked_by_min", label: "All Blocked", color: "bg-destructive/10 text-destructive border-destructive/30", activeColor: "bg-destructive text-white shadow-md shadow-destructive/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
                { value: "profit_guard_block", label: "Profit Guard", color: "bg-amber-100 text-amber-800 border-amber-300", activeColor: "bg-amber-600 text-white shadow-md shadow-amber-600/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
                { value: "bb_suppressed", label: "BB Suppressed", color: "bg-blue-100 text-blue-800 border-blue-300", activeColor: "bg-blue-500 text-white shadow-md shadow-blue-500/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
              ] as const;
              const automationStates = [
                { value: "blocked_auto", label: "Auto-Handling", color: "bg-slate-100 text-slate-700 border-slate-300", activeColor: "bg-muted-foreground text-white shadow-md", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
                { value: "NONE", label: "Healthy", color: "bg-emerald-100 text-emerald-800 border-emerald-300", activeColor: "bg-emerald-500 text-white shadow-md shadow-emerald-500/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
              ] as const;
              const analytics = [
                { value: "no_sales_30d", label: "🟠 No Sales 30d", color: "bg-orange-100 text-orange-800 border-orange-300", activeColor: "bg-orange-500 text-white shadow-md shadow-orange-500/25", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
              ] as const;
              const advanced = [
                { value: "HAS_ANY", label: "Has Alert", color: "bg-amber-100 text-amber-800 border-amber-300", activeColor: "bg-amber-500 text-white shadow-md shadow-amber-500/25", icon: <AlertTriangle className="h-3 w-3" /> },
                { value: "blocked_needs_you", label: "0 Sales (7d)", color: "bg-slate-100 text-slate-700 border-slate-300", activeColor: "bg-foreground text-background shadow-md", icon: <span className="h-2 w-2 rounded-full bg-current" /> },
              ] as const;
              ...
              >
                  ...
                  <TableBody>
                    {paginatedItems.map((item, index) => {
                      ...
                      // Row tint uses the dedicated --row-tint tokens, not --shipment-row —
                      // the grid's chrome (surface/elevated header/control inputs) stays on
                      // --shipment-* per Option A, but individual record rows get their own
                      // light tint so they read as distinct cards on the light page.
                      const rowTone = index % 2 === 0 ? 'bg-row-tint' : 'bg-row-tint-alt';

                      return (
                        <TableRow
                          key={item.id}
                          className={`text-xs border-b border-border/60 hover:!bg-row-tint-hover transition-colors duration-200 text-foreground ${rowTone} ${isHiddenRow ? 'opacity-70' : ''}`}
                        >
                          {/* Checkbox */}
